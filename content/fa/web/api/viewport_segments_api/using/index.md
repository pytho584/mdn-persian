...
source: "https://developer.mozilla.org/en-US/docs/Web/API/Viewport_segments_API/Using"
...
To be fit a layout precisely into the available viewport segments, the [viewport segment environment variables](/en-US/docs/Web/CSS/Reference/Values/env#viewport-segment-width) provide access to each segment's dimensions and placement inside the overall viewport. The browser provides [environment variables] enabling access to each segment's width and height and the offset positions of its top, right, bottom, and left edges:
