...
source: "https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/input/week"
...
![تصویری از یک input با متن 'week 01, 2017'. پس‌زمینه 2017 مانند حلقه فوکوس آبی است. سه آیکون در input وجود دارد: x یا پاک کردن، یک spinner با فلش‌های کوچک بالا و پایین، و یک فلش بزرگ رو به پایین. یک تقویم به صورت پاپ‌آپ در زیر input قرار دارد که ماه January 2017 را نشان می‌دهد. ستون اول تقویم هفته است: 52, 1, 2, 3, 4, 5. تقویم کامل ماه سمت راست آن قرار دارد. سطر هفته 1 و روزهای 2 تا 8 January هایلایت شده است. در همان خط ماه، دکمه‌هایی برای حرکت به راست و چپ برای ماه‌های بعدی و قبلی وجود دارد.](week-control-chrome.png)
...
آخرین (از نظر زمانی) سال و شماره هفته، به رشته‌ای که در بخش [مقدار](#value) توضیح داده شد. اگر [`value`](/en-US/docs/Web/HTML/Reference/Elements/input#value) وارد شده از این مقدار بیشتر باشد، عنصر در [اعتبارسنجی محدودیت](/en-US/docs/Web/HTML/Guides/Constraint_validation) شکست می‌خورد. اگر مقدار `max` یک رشته هفته معتبر نباشد، عنصر حداکثر مقداری نخواهد داشت.
...
> [!NOTE]
> از آنجا که یک فیلد read-only نمی‌تواند مقداری داشته باشد، `required` هیچ اثری بر ورودی‌هایی که ویژگی `readonly` نیز دارند ندارد.

### step

ویژگی `step` عددی است که میزان دانه‌بندی (granularity) مقداری که باید رعایت شود را مشخص می‌کند. همچنین می‌تواند مقدار ویژه‌ی `any` باشد که در ادامه توضیح داده شده است. فقط مقادیری معتبر هستند که مضربی صحیح از step نسبت به پایه‌ی step باشند. پایه‌ی step برابر است با [`min`](#min) اگر مشخص شده باشد، در غیر این صورت [`value`](/en-US/docs/Web/HTML/Reference/Elements/input#value)، و اگر هیچ‌کدام مشخص نشده باشد، ‎−259,200,000 (آغاز هفته‌ی `1970-W01`).
...
مقدار رشته‌ای `any` به این معنی است که هیچ گام (step) اجباری نیست و هر مقداری مجاز است (به جز سایر محدودیت‌ها مثل [`min`](#min) و [`max`](#max)). در عمل، برای ورودی‌های `week` همان اثر `1` را دارد، زیرا رابط کاربری انتخابگر فقط انتخاب هفته‌های کامل را اجازه می‌دهد.

> [!NOTE]
> وقتی داده‌ی واردشده توسط کاربر با پیکربندی گام مطابقت نداشته باشد، عامل کاربر (user agent) ممکن است مقدار را به نزدیک‌ترین مقدار معتبر گرد کند و در صورت وجود دو گزینه‌ی به یک اندازه نزدیک، اعداد مثبت را ترجیح دهد.
...
ما به کاربردهای پایه و پیچیده‌تر `<input type="week">` خواهیم پرداخت و سپس پیشنهادهایی برای کاهش مشکل پشتیبانی مرورگر ارائه خواهیم داد (به [مدیریت پشتیبانی مرورگر](#handling_browser_support) مراجعه کنید).
...
![یک پنجرهٔ مودال. عنوان آن «set week» است. دو ستون وجود دارد: سمت چپ عدد ۳۶ در وسط با شفافیت کامل، بالای آن ۳۵ و پایین آن ۳۷ به‌صورت نیمه‌شفاف دیده می‌شود. در سمت راست، ۲۰۱۷ کاملاً واضح است. گزینهٔ دیگری وجود ندارد. سه لینک متنی یا دکمه در پایین دیده می‌شود: «clear» در چپ و «cancel» و «set» در راست.](week-chrome-android.png)
...
> [!NOTE]
> به یاد داشته باشید که بعضی سال‌ها ۵۳ هفته دارند (به [Weeks per year](https://en.wikipedia.org/wiki/ISO_week_date#Weeks_per_year) مراجعه کنید). در برنامه‌های واقعی حتماً این نکته را در نظر بگیرید.
...
<table class="properties">
  <tbody>
    <tr>
      <td><strong>مقدار (Value)</strong></td>
      <td>
        یک رشته (string) که یک هفته و سال را مشخص می‌کند، یا خالی
      </td>
    </tr>
    <tr>
