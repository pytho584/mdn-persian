...
- `char` (منسوخ)
  - : کار خاصی انجام نمی‌دهد. در اصل برای تعیین ترازبندی محتوای سلول نسبت به یک نویسهٔ خاص در نظر گرفته شده بود. مقادیر معمول شامل نقطه (`.`) برای تراز کردن اعداد یا مقادیر پولی است. اگر [`align`](#align) روی `char` تنظیم نشده باشد، این attribute نادیده گرفته می‌شود.

- `charoff` (منسوخ)
  - : کار خاصی انجام نمی‌دهد. در اصل برای تعیین تعداد کاراکترهایی بود که محتوای سلول از نویسهٔ تراز (مشخص‌شده توسط attribute [`char`](#char)) جابه‌جا شود.
...
- هنگام استفاده از attributeهای [`colspan`](#colspan) و [`rowspan`](#rowspan) برای گسترش سلول‌های داده در چند ستون و سطر، سلول‌هایی که این attributeها را ندارند (با مقدار پیش‌فرض `1`) به‌طور خودکار در فضاهای خالی باقی‌مانده در ساختار جدول قرار می‌گیرند؛ این سلول‌ها یک سلول 1×1 را اشغال می‌کنند، همان‌طور که در شکل زیر نشان داده شده است:

  ![تصویری که گسترش ستون و سطر سلول‌های جدول را نشان می‌دهد: سلول‌های ۱، ۳ و ۴ دو سطر را اشغال می‌کنند؛ سلول ۲ دو ستون را اشغال می‌کند؛ سلول‌های ۵ و ۶ در سلول‌های خالی موجود که ستون دوم و سوم در سطر دوم هستند جای می‌گیرند](/shared-assets/images/diagrams/html/table/column-row-span.png)

  > [!NOTE]
  > این attributeها نباید برای همپوشانی سلول‌ها استفاده شوند.
...
> [!NOTE]
> به‌طور معمول، یک گروه سربرگ جدول با سربرگ‌های ستون برای درک بهتر اطلاعات هر ستون پیاده‌سازی می‌شود. عناصر `<thead>` و `<tbody>` برای گروه‌بندی این سطرهای سربرگ‌ها و داده‌ها در بخش‌های سربرگ و بدنهٔ جدول استفاده می‌شوند. این کار در این مثال انجام نشده است تا تمرکز روی سلول‌های داده باشد و پیچیدگی مثال کمتر شود.
...
این مثال، جدول پایه را از [مثال قبلی](#basic_data_cells) گسترش می‌دهد و یک سلول اضافی «ABC» به آن اضافه می‌کند.
...
اگرچه [نمایش بصری](#result_2) نسبت به [جدول مثال قبلی](#column_and_row_spanning) تغییری نکرده، اما اکنون هر سلول داده (`<td>`) به‌طور صریح با سلول سرستون ردیف خود (`<th>`) مرتبط شده است.
