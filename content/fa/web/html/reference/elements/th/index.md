...
- `align` (منسوخ شده)
  - : تراز افقی سلول سربرگ را مشخص می‌کند. مقادیر شمارشی (enumerated) ممکن عبارت‌اند از `left`، `center`، `right`، `justify` و `char`. در صورت پشتیبانی، مقدار `char` محتوای متنی را بر اساس کاراکتری که در ویژگی [`char`](#char) تعریف شده و انحراف (offset) مشخص‌شده توسط ویژگی [`charoff`](#charoff) تراز می‌کند. به‌جای این ویژگی (که منسوخ شده) از ویژگی CSS `text-align` استفاده کنید.

- `axis` (منسوخ شده)
  - : شامل فهرستی از رشته‌ها با جداکننده فاصله است که هر کدام با ویژگی `id` گروهی از سلول‌ها مطابقت دارد که سلول سربرگ به آن‌ها اعمال می‌شود. به‌جای این ویژگی (که منسوخ شده) از ویژگی [`scope`](#scope) استفاده کنید.
...
- `char` (منسوخ شده)
  - : هیچ کاری انجام نمی‌دهد. در ابتدا برای مشخص کردن تراز محتوا نسبت به یک کاراکتر از سلول سربرگ در نظر گرفته شده بود. مقادیر معمول برای آن شامل نقطه (`.`) است، زمانی که بخواهید اعداد یا مقادیر پولی را تراز کنید. اگر [`align`](#align) روی `char` تنظیم نشده باشد، این ویژگی نادیده گرفته می‌شود.

- `charoff` (منسوخ شده)
  - : هیچ کاری انجام نمی‌دهد. در ابتدا برای مشخص کردن تعداد کاراکترهایی که محتوای سلول سربرگ از کاراکتر تراز (مشخص‌شده توسط ویژگی [`char`](#char)) فاصله بگیرد، در نظر گرفته شده بود.
...
- در زمینه‌های ساده، استفاده از ویژگی [`scope`](#scope) روی سلول‌های سربرگ (`<th>`) اضافی است؛ زیرا [`scope`](#scope) به‌صورت ضمنی تعیین می‌شود. با این حال، برخی فناوری‌های کمکی ممکن است نتوانند به‌درستی آن را تشخیص دهند، بنابراین مشخص‌کردن scope برای سربرگ می‌تواند تجربه کاربری را بهبود بخشد.
- هنگام استفاده از ویژگی‌های [`colspan`](#colspan) و [`rowspan`](#rowspan) برای گسترش سلول‌های سربرگ در چند ستون و ردیف، سلول‌هایی که این ویژگی‌ها برای آن‌ها تعریف نشده است (با مقدار پیش‌فرض `1`)، به‌طور خودکار در فضاهای خالی موجود در ساختار جدول که سلول‌های 1×1 را پوشش می‌دهند جای می‌گیرند؛ همان‌طور که در تصویر زیر نشان داده شده است:

  ![تصویری که گسترش ستون و ردیف سلول‌های جدول را نشان می‌دهد: سلول‌های ۱، ۳ و ۴ دو ردیف را پوشش می‌دهند؛ سلول ۲ دو ستون را پوشش می‌دهد؛ سلول‌های ۵ و ۶ در سلول‌های موجود که ستون دوم و سوم در ردیف دوم هستند قرار می‌گیرند](/shared-assets/images/diagrams/html/table/column-row-span.png)

  > [!NOTE]
  > از این ویژگی‌ها نباید برای همپوشانی سلول‌ها استفاده شود.
...
ردیف اول (عنصر `<tr>`) شامل سرستون‌ها (عناصر `<th>`) است که به عنوان «عنوان» ستون‌ها عمل می‌کنند تا درک اطلاعات هر ستون و شناسایی داده‌ها آسان‌تر شود. برای مشخص کردن اینکه هر سرستون به تمام سلول‌های ستون مربوطه ارتباط دارد، ویژگی [`scope`](#scope) روی `col` (ستون) تنظیم شده است.

ردیف‌های باقی‌مانده حاوی داده‌های اصلی جدول هستند. هر یک از این ردیف‌ها یک سرسطر (عنصر `<th>`) دارد که به عنوان اولین سلول قرار می‌گیرد. این کار یک ستون از سرسطرها را به عنوان اولین ستون جدول ایجاد می‌کند. مشابه سرستون‌ها، ویژگی [`scope`](#scope) روی `row` تنظیم می‌شود تا مشخص کند هر سرسطر به کدام سلول‌ها مربوط است؛ که در مثال زیر، تمام سلول‌های داده (عناصر `<td>`) در هر ردیف هستند.

> **نکته:**
> معمولاً از عناصر گروه‌بندی `<thead>` و `<tbody>` برای دسته‌بندی ردیف‌های دارای header در بخش‌های head و body جدول استفاده می‌شود. این عناصر در این مثال برای کاهش پیچیدگی و تمرکز روی استفاده از سلول‌های header حذف شده‌اند.
...
برای استایل‌دهی جدول و سلول‌های آن از CSS پایه استفاده شده است. از [attribute selector](/en-US/docs/Web/CSS/Reference/Selectors/Attribute_selectors)های CSS برای هدف قرار دادن سلول‌های header بر اساس مقدار ویژگی [`scope`](#scope) استفاده می‌کنیم، تا سرستون‌ها و سرسطرها (عناصر `<th>`) برجسته شوند و از یکدیگر و از سلول‌های داده (عناصر `<td>`) متمایز گردند.
...
این مثال جدول پایه‌ای از [مثال قبلی](#basic_column_and_row_headers) را با افزودن یک ردیف دوم برای سرستون‌های اضافی گسترش و بهبود می‌بخشد.
...
همان‌طور که در [یادداشت‌های استفاده](#usage_notes) نشان داده شده، ویژگی‌های [`colspan`](#colspan) و [`rowspan`](#rowspan) را می‌توان برای عناصر `<th>` به کار برد تا سلول‌های header را به ستون‌ها و ردیف‌های درست اختصاص دهند. برای ایجاد یک header «دو ردیفی» در ساختار جدول، دو سلول header اول در اولین عنصر `<tr>` در دو ردیف ادغام می‌شوند. سومین سلول header در دو ستون گسترش می‌یابد (در همان ردیف اول باقی می‌ماند). این تنظیم دو ناحیه خالی در ستون‌های سوم و چهارم در ردیف دوم ایجاد می‌کند، جایی که دو header درون دومین عنصر `<tr>` به طور خودکار قرار می‌گیرند، با مقدار پیش‌فرض `1` برای ویژگی‌های [`colspan`](#colspan) و [`rowspan`](#rowspan).

> [!NOTE]
> معمولاً از عناصر `<thead>` و `<tbody>` برای گروه‌بندی سطرهایی که شامل هدر هستند به بخش‌های سربرگ و بدنهٔ جدول استفاده می‌شود. در این مثال، برای تمرکز روی هدرها و ادغام سلول‌ها و کاهش پیچیدگی، از این عناصر استفاده نشده است.
...
CSS این مثال بدون تغییر از [مثال قبلی](#basic_column_and_row_headers) باقی مانده است.
...
برای روابط پیچیده‌تر بین سلول‌های هدر، استفاده از عناصر `th` تنها با ویژگی [`scope`](#scope) ممکن است برای فناوری‌های کمکی، به‌ویژه صفحه‌خوان‌ها، کافی نباشد.

#### HTML

برای بهبود دسترس‌پذیری (accessibility) [مثال قبلی](#column_and_row_spanning) و همچنین امکان بیان هدرهای مرتبط با هر سلول هدر توسط صفحه‌خوان‌ها، می‌توان از ویژگی [`headers`](#headers) به همراه ویژگی‌های [`id`](/en-US/docs/Web/HTML/Reference/Global_attributes/id) استفاده کرد. در این مثال، چون ستون «Pronunciation» به دو ستون تقسیم شده و یک هدر دو ردیفه ایجاد شده، فناوری‌های کمکی مانند صفحه‌خوان‌ها ممکن است نتوانند تشخیص دهند که سلول هدر «Pronunciation» با کدام سلول‌های هدر دیگر در ارتباط است و برعکس. بنابراین، ویژگی [`headers`](#headers) روی سلول‌های هدر «Pronunciation»، «IPA» و «Respelling» استفاده شده است تا این سلول‌ها بر اساس مقادیر شناسه‌های یکتای موجود در ویژگی‌های [`id`](/en-US/docs/Web/HTML/Reference/Global_attributes/id) — که به صورت فهرستی با جداکنندهٔ فاصله نوشته می‌شوند — به یکدیگر مرتبط شوند.

> [!NOTE]
> توصیه می‌شود برای ویژگی [`id`](/en-US/docs/Web/HTML/Reference/Global_attributes/id) از مقادیر توصیفی‌تر و کاربردی‌تر استفاده کنید. هر `id` در یک سند باید برای همان سند یکتا باشد. در این مثال، مقادیر `id` به‌صورت کاراکترهای تکی هستند تا تمرکز روی مفهوم ویژگی [`headers`](#headers) حفظ شود.
...
[نتیجهٔ بصری](#result_2) نسبت به [جدول مثال قبلی](#column_and_row_spanning) تغییری نکرده است.
    ...
    <tr>
      ...
      <td>
        <a href="/en-US/docs/Web/HTML/Guides/Content_categories#flow_content"
          >محتواهای جریانی</a
        >، به شرطی که هیچ فرزند header، footer، محتوای بخش‌بندی یا محتوای عنوان‌دار نداشته باشد.
      </td>
    </tr>
    <tr>
      <th scope="row">حذف تگ</th>
      <td>
        تگ شروع اجباری است.<br />تگ پایان را می‌توان حذف کرد، اگر بلافاصله بعد از آن یک عنصر
        <code>&lt;th&gt;</code> یا
        <code>&lt;td&gt;</code> بیاید، یا اگر داده‌ی دیگری در عنصر والد وجود نداشته باشد.
      </td>
    </tr>
    <tr>
      <th scope="row">والدین مجاز</th>
      <td>یک عنصر <code>&lt;tr&gt;</code></td>
    </tr>
    <tr>
      <th scope="row">نقش ARIA ضمنی</th>
      <td>
        <a href="/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/columnheader_role"><code>columnheader</code></a> یا <a href="/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/rowheader_role"><code>rowheader</code></a>
      </td>
    </tr>
    <tr>
