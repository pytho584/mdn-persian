...
- `border` (منسوخ شده)
  - : اندازه قاب دور جدول را بر حسب پیکسل و به صورت یک عدد صحیح غیرمنفی مشخص می‌کرد. اگر مقدار `0` می‌گرفت، ویژگی [`frame`](#frame) به `void` تنظیم می‌شد. به‌جای آن از خصوصیت CSS {{cssxref("border")}} استفاده کنید.
...
این مثال [جدول پایه](#basic_table) را گسترش می‌دهد، محتوا را افزایش داده و استایل‌های CSS پایه را اضافه می‌کند.
...
> [!NOTE]
> اگر ساختار جدول پیچیده‌تر باشد، استفاده (اضافی) از ویژگی `headers` روی المان‌های `<th>` و `<td>` می‌تواند دسترس‌پذیری را بهبود ببخشد و به فناوری‌های کمکی در شناسایی رابطه بین سلول‌ها کمک کند؛ به [جدول‌های پیچیده](#complicated_tables) مراجعه کنید.
...
خلاصه جدول با استفاده از یک [caption](#captions) (عنصر `<caption>`) به عنوان اولین فرزند `<table>` اضافه می‌شود. این کپشن، نام قابل‌دسترس (accessible name) یا توضیح قابل‌دسترس (accessible description) جدول را فراهم می‌کند.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        به این ترتیب:
        <ol>
          <li>یک عنصر اختیاری `<caption>`</li>
          <li>صفر یا چند عنصر `<colgroup>`</li>
          <li>یک عنصر اختیاری `<thead>`</li>
          <li>
            یکی از موارد زیر:
            <ul>
              <li>صفر یا چند عنصر `<tbody>`</li>
              <li>یک یا چند عنصر `<tr>`</li>
            </ul>
          </li>
          <li>یک عنصر اختیاری `<tfoot>`</li>
        </ol>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>هر عنصری که محتوای جریانی (flow content) را بپذیرد.</td>
    </tr>
    <tr>
